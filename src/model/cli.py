...
class NNCLI:
    ...
    def prompt_hidden_layers(self):
        while True:
            try:
                self.n_layers = int(input('How many hidden layers? [ Hidden layers are the layers in the network between in and output layers ] '))
                if self.n_layers <= 0:
                    raise ValueError("Number of hidden layers must be a positive integer.")
                break
            except ValueError:
    ...
    def prompt_hidden_layer_sizes(self):
        for i in range(1, self.n_layers + 1):
            while True:
                try:
                    layer = int(input(f"Size of the ({i}.) layer? [ Meaning the number of node in layer {i} ] "))
                    if layer <= 0:
                        raise ValueError("Size of each hidden layer must be a positive integer.")
    ...
    def prompt_batch_size(self):
        while True:
            try:
                self.batchsize = int(input('Size of a batch? [ Training is divided to batches , bigger batch meaning faster training but less accuracy per step ] '))
                if self.batchsize <= 0:
    ...
    def prompt_epochs(self):
        while True:
            try:
                self.Epocs = int(input('Number of epochs? [ One epoch meaning one full training cycle with the whole training dataset ] '))
                if self.Epocs <= 0:
    ...
    def prompt_activation_func(self):
        while True:
            self.actication_func = input('Activation function? Choose [ReLU, Sigmoid]: [ This is math funct to add non-linearity to the nodes of the network ] ')
            if self.actication_func.lower() in ['relu', 'sigmoid']:
                break
            else:
                print("Please choose a valid activation function [ReLU, Sigmoid].")
    ...
    def train(self):
        ...
        print(f'<< alpha = {self.learningrate}, batchsize = {self.batchsize}, epocs = {self.Epocs}, activation function = {self.actication_func} <<')
